...
public class ShopTextOutput {
    ...
    /**
     * Checks whether a passed item is non-null and tradable. Null values are recognised as
     * untradable. If the item is not tradable, sends an informational message to passed command
     * sender.
     *
    ...
    /**
     * Checks whether a passed item is non-null and tradable for a given transaction type. Null
     * values are recognised as untradable. If the item is not tradable, sends an informational
     * message to passed command sender.
     *
     * @param receiver  the receiver of the possible message
     * @param item      the item to check
     * @param queryInfo an additional string describing the item, used when untradable, may be null
     * @param type      the transaction type to check for, or null to check if any trade is
     *                  possible
     * @return whether the item is tradable
     */
    public boolean checkTradable(@Nonnull CommandSender receiver, @Nullable ShopItem item, @Nullable String queryInfo,
    ...
    /**
     * Checks whether given item is null, and issues a warning message to given receiver stating
     * that such item does not exist if so. Does nothing if given item is non-null.
     *
     * @param receiver  the receiver of the message
    ...
    /**
     * Sends information about the price of specified amount of shop items. If amount is 1, sends
     * only the price for a single item. Otherwise, sends the price for specified amount of items
     * too.
     *
     ...
     * @param queryInfo an additional string describing the item, used when item is null, may be
     *                  null
     */
    ...
    /**
     * Sends information about the price of a single piece of a shop item to a command sender. This
     * handles null values as untradable items.
     *
     * @param receiver  the receiver of the messages
     * @param item      the item to display, may be null
     * @param queryInfo an additional string describing the item, used when item is null, may be
    ...
    /**
     * Sends information about the price of a specific amount of a specific shop item to a command
     * sender. This handles null values as untradeable items.
     *
     * @param receiver  the receiver of the messages
     * @param item      the item to display, may be null
     * @param amount    the amount of the item to display
    ...
    /**
     * Notifies a command sender about a successful transaction initiated by them with information
    ...
    public void announceDiscount(ShopItem item) {
        ...
        BaseComponent[] notification = module.getPrefixBuilder()
                ...
                .tooltip("Nur für kurze Zeit!!!1")
                .append(item.getDisplayName(), ChatColor.YELLOW).bold(false)
                .event(createItemHover(item))
                .command("/shop preis " + item.getSerializationName())
