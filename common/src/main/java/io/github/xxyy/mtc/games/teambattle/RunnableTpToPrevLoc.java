...
public final class RunnableTpToPrevLoc implements Runnable {
	...
	public void run() {
		if(helper.plr.getLocation().distanceSquared(helper.startLoc) > 0.5){
			helper.plr.sendMessage(TeamBattle.CHAT_PREFIX +" §oDu kannst echt nicht 2 Sekunden stillhalten! Daher kannst du nicht teleportiert werden :(");
			return;
		}
		if(helper.plr.getHealth() != helper.startHealth){
			helper.plr.sendMessage(TeamBattle.CHAT_PREFIX +" §oNa toll, jetzt hast du während der 2 Sekunden Schaden erhalten. Daher kannst du nicht teleportiert werden :(");
			return;
		}
		helper.doTpPrev();
	}
