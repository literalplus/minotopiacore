...
import io.github.xxyy.mtc.misc.BungeeHelper;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (StringUtils.isNumeric(args[1])) {
            try {
                ...
                sender.sendMessage(MTC.warnChatPrefix + "Das ist keine Zahl...aber irgendwie schon...ZAHLCEPTION! (zu gross?)");
                return true;
            }
            reasonStartIndex = 2;
            if (args.length < 3) {
        ...
        if (multiplier > 15) {
            sender.sendMessage(MTC.warnChatPrefix + "Das sind aber viele Warns...Auf 15 reduziert.");
            multiplier = 1;
        }
        Player plr = Bukkit.getPlayerExact(args[0]); //REFACTOR
        if (plr == null) {
        ...
        if (WarnHelper.playerTimeouts.contains(args[0].toLowerCase())) {
            sender.sendMessage(MTC.warnChatPrefix + "Bitte warte 20 Sekunden, bevor du diesen Spieler erneut warnen kannst.");
            return true;
        }
