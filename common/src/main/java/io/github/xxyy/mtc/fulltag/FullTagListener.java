...
public final class FullTagListener implements Listener {
    public static boolean doLogic(int id, FullInfo fi, Inventory inv, String plrName, String action, Location loc) {
        ...
        if (inv != null) {
            fi.inEnderchest = inv.getType() == InventoryType.ENDER_CHEST;
        }
        if (loc != null) {
            fi.x = loc.getBlockX();
            fi.y = loc.getBlockY();
...
//            fi.nullify(); //Don't delete fulls, for FullReturn.
        ...
        switch (e.getAction()) {
            ...
            case CLONE_STACK:
                ...
                if (who instanceof Player) {
                    who.sendMessage(MTC.chatPrefix + "§cFulls werden nicht dupliziert!");
                }
                return;
            default:
        }
...
//        case QUICKBAR:
            ...
            default:
                ...
                if (FullTagListener.doLogic(e, inv, "action_" + e.getAction() + "_" + e.getSlotType().name() + "_open_"
                    ...
                    if (who instanceof Player) {
                        who.sendMessage(MTC.chatPrefix + "§cDie Full, die du angeklickt hast, ist unbekannt und wurde daher entfernt. §eFür Beschwerden notiere die bitte unbedingt die aktuelle Uhrzeit!");
                    }
