...
public final class CommandListWarns implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0) {
            ...
            if (!sender.hasPermission("mtc.warns.list.others")) {
                sender.sendMessage(MTC.warnChatPrefix + "§cDu darfst nur deine eigenen Warns ansehen! /warns");
                return true;
            }
            listWarnsOfTo(sender, args[0]);
            return true;
        } else {
    ...
    public void listWarnsOfTo(CommandSender sender, String targetName) {
        ...
        for (WarnInfo wi : warns) {//ordered by ID in WarnInfo method
            if (wi.id <= 0) {
                if (wi.id == -4) {
                    sender.sendMessage("§6Du hast keine Warnungen =)");
                    return;
                }
                sender.sendMessage("§4Fehler: " + wi.id);
                continue;
            }
