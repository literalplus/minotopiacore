...
public class CommandDeleteWarn implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if(args.length <= 0 || args[0].equalsIgnoreCase("help")){
			this.printHelpTo(sender); return true;
		}
		String targetName = args[0];
		boolean forceRemove = false;
		boolean unknownReason = false;
		boolean resetFlag = false;
		byte removeCount = -127;
		byte ctr = 0; //'i' is overused in this class already :P
		for(String arg : args){
			if(arg.startsWith("-")){
				if(arg.equalsIgnoreCase("-f")){
					if(!sender.hasPermission("mtc.warns.remove.force")){ sender.sendMessage(MTC.warnChatPrefix+"§cDu darfst Warnungen nicht permanent löschen!"); return true; }
					forceRemove = true;
					unknownReason = false;
					ctr++; continue;
					...
					if(forceRemove){ ctr++; continue; }//forceremove overrides
					unknownReason = true;
					ctr++; continue;
					...
					try {
						...
						sender.sendMessage(MTC.warnChatPrefix+"§cInvalides Argument: §oEindeutige ID."); this.printHelpTo(sender); return true;
					}
					WarnInfo wi = WarnInfo.getById(uid);
					if(wi.id < 0){ sender.sendMessage(MTC.warnChatPrefix+"Fehler: "+wi.id); return true; }
					if(resetFlag) { wi.status = 0; wi.flush(); }
					else if(forceRemove) {
                        wi.nullify();
					...
					sender.sendMessage(MTC.warnChatPrefix+"§aWarn §e"+uid+"§a erfolgrich entfernt."); return true;
		...
		if(warns.get(0).id == -4){ sender.sendMessage(MTC.warnChatPrefix+"§e"+args[0]+"§a hat keine Warnungen!"); return true; }//there will always be an item in the list
		if(warns.get(0).id < 0){ sender.sendMessage(MTC.warnChatPrefix+"Fehler: "+warns.get(0).id); return true; }//there will always be an item in the list
		if(forceRemove){
