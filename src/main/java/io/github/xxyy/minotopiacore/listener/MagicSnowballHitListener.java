...
public class MagicSnowballHitListener implements Listener {
    ...
    @EventHandler(priority=EventPriority.HIGH)// this HAS to ignore if the damage was already cancelled by ClanDamage.. and MUST therefore be called afterwards...
    public void onDmg(EntityDamageByEntityEvent e){
        if(e.isCancelled()) return;
        if(!e.getEntityType().equals(EntityType.PLAYER) || !e.getDamager().getType().equals(EntityType.SNOWBALL)) return;
        Player plr = (Player)e.getEntity();
        Snowball ball = (Snowball)e.getDamager();
        LivingEntity leShooter = ball.getShooter();//le = LivingEntity
        if(!(leShooter instanceof Player)) return; //Snow golems
        Player plrShooter = (Player)leShooter;
