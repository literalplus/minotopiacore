...
public class ClanInfo {
    ...
    public String motd="The cake is a lie!";
    ...
    private ClanInfo(int id, String name, String prefix, String leaderName, Location base,
            String motd, int money, short level, int kills, int deaths){
        this.id=id; this.name=name; this.prefix=prefix; this.leaderName=leaderName;
        this.base=base; this.motd=motd; this.money=money; this.level=level;
     ...
     * @return see {@link ClanInfo#create(String, String, String, Location, String, int, short, int, int)}
     * @see ClanInfo#create(String, String, String, Location, String, int, short, int, int)
     * @author xxyy98<xxyy98@gmail.com
     */
    public static ClanInfo create(String name, String prefix, Player leader){
        return ClanInfo.create(name, prefix, leader.getName(), leader.getLocation(),
                "Hier könnte eine MotD stehen!", 0, (short)0, 0, 0);
    }
    /**
    ...
    protected static ClanInfo getById(int id){
        ...
        SafeSql sql = MTC.instance().ssql;
        if(sql == null) return new ClanInfo(-2);
        ResultSet rs = sql.safelyExecuteQuery("SELECT * FROM "+sql.dbName+"."+Const.TABLE_CLANS+" WHERE id=?", id);
        try {
    ...
    protected static ClanInfo getByName(String name){
        SafeSql sql = MTC.instance().ssql;
        if(sql == null) return new ClanInfo(-2);
        ResultSet rs = sql.safelyExecuteQuery("SELECT * FROM "+sql.dbName+"."+Const.TABLE_CLANS+" WHERE name=?", name);
        try {
            if(rs == null || !rs.isBeforeFirst()) return new ClanInfo(-3);
            rs.next();
            return new ClanInfo(rs.getInt("id"), rs.getString("name"), rs.getString("prefix"),
    ...
    protected static ClanInfo getByPrefix(String prefix){
        SafeSql sql = MTC.instance().ssql;
        if(sql == null) return new ClanInfo(-2);
        ResultSet rs = sql.safelyExecuteQuery("SELECT * FROM "+sql.dbName+"."+Const.TABLE_CLANS+" WHERE prefix=?", prefix);
        try {
            if(rs == null || !rs.isBeforeFirst()) return new ClanInfo(-3);
            rs.next();
            return new ClanInfo(rs.getInt("id"), rs.getString("name"), rs.getString("prefix"),
