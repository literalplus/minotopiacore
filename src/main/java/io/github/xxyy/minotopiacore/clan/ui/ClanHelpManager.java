...
public class ClanHelpManager {
    ...
    public static HashMap<String,ClanHelpManager> helpMans = new HashMap<>();
    
    public ClanHelpManager(String cmdName,String[] cmdDescLines,HashMap<String,String> subCmds){
    ...
    public void printHelpToSender(CommandSender sender,String label,String pageNum,String helpCmdLabel){
        //6 Commands/page
        int intPageNum; int Pages=1; int i=0;
        if(this.subCmds.size() > 6){
            if((this.subCmds.size() % 6) == 0) {
                Pages=this.subCmds.size()/6;
            } else {
                Pages=(this.subCmds.size()/6) + 1;
            }
        }
        if(pageNum.isEmpty()){
            intPageNum=1;
        ...
        //render begin
        sender.sendMessage(String.format(MTCHelper.loc("XC-helpheader", sender.getName(), false),this.helpPageTitle,intPageNum,Pages));
        //command description
