...
public class CommandPeace extends MTCPlayerOnlyCommandExecutor implements TabCompleter
{
    ...
    {
        if(args.length == 0) return MTCHelper.sendLocArgs("XU-peacehelp", plr, false, label);
        switch(args[0]){
        case "list":
            short perPage = 15;
            PeaceInfo piList = PeaceInfo.get(plrName);
            if(piList.errCode < 0) return MTCHelper.sendLoc("XU-nopeace", plr, true);
            int rowstart = 0;
            int page = 1;
            ...
            return CommandPeace.sendPeaceList(plr, piList.peacedPlrs, rowstart, perPage, label, page + 1);
        case "status":
            if(args.length < 2) return MTCHelper.sendLocArgs("XU-peacehelp", plr, false, label);
            if(args[1].equalsIgnoreCase(plrName)) return CommandHelper.msg(MTC.chatPrefix+"Ich nehme an, dass du mit dir selbst Frieden hast.", plr);
            if(PeaceInfo.isInPeaceWith(plrName, args[1])) return MTCHelper.sendLocArgs("XU-plrpeace", plr, true, args[1]);
            if(PeaceInfo.hasRequest(plrName, args[1])) return MTCHelper.sendLocArgs("XU-preqpending", plr, true, args[1]);
            if(PeaceInfo.hasRequest(args[1], plrName)) return MTCHelper.sendLocArgs("XU-preqpendingown", plr, true, args[1]);
            return MTCHelper.sendLocArgs("XU-plrnopeace", plr, true, args[1]);
        case "help":
            return MTCHelper.sendLocArgs("XU-peacehelp", plr, false, label);
        case "revoke":
            if(args.length < 2) return MTCHelper.sendLocArgs("XU-peacehelp", plr, false, label);
            if(args[1].equalsIgnoreCase(plrName)) return CommandHelper.msg(MTC.chatPrefix+"Du hast dir selbst keine Friedensanfrage gesendet.", plr);
            if(!PeaceInfo.hasRequest(plrName, args[1])) return MTCHelper.sendLocArgs("XU-nopreqsent", plr, true, args[1]);
        ...
        default:
            if(args[0].equalsIgnoreCase(plrName)) return CommandHelper.msg(MTC.chatPrefix+"Du kannst nicht mit dir selbst Frieden schliessen :)", plr);
             PeaceInfo piDefault = PeaceInfo.get(plrName);
             if(piDefault.errCode < 0 && piDefault.errCode != -4) return CommandHelper.msg("§cGenerischer SQL-Fehler: "+piDefault.errCode, plr);
             ...
             if(PeaceInfo.hasRequest(plrName, args[0])) return MTCHelper.sendLocArgs("XU-palreadysent", plr, true, args[0]); //already sent request
    ...
    {
        if(!(sender instanceof Player)) return Lists.newArrayList("Du kannst diesen Befehl nur als Spieler ausfuehren!!!11");
        if(args.length == 0){
            List<String> lst = CommandHelper.getOnlinePlayerNames();
            lst.add("list"); lst.add("status"); lst.add("help");
            lst.add("revoke");
            return lst;
        }else if(args[0].equalsIgnoreCase("revoke")){
            PeaceInfo pi = PeaceInfo.get(sender.getName());
            if(pi.errCode < 0) return null;
    ...
    public static String getPlayerString(String plrName){
        ...
        if(plr == null) return MTCHelper.locArgs("XC-membersoff", "CONSOLE", false, plrName);//continuity
        return MTCHelper.locArgs("XC-memberson", "CONSOLE", false, plrName);
    }
    
    private static boolean sendPeaceList(CommandSender sender, List<String> lst, int rowstart, short perPage, String label, int nextPage){
        String toSend = "";
        ...
        if(i < lst.size()) return MTCHelper.sendLocArgs("XU-pmorepages", sender, false, label, nextPage);
