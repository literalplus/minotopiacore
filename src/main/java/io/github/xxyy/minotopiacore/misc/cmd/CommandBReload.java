...
public class CommandBReload extends MTCCommandExecutor {
    ...
    public boolean catchCommand(CommandSender sender, String senderName, Command cmd, String label, String[] args) {
        ...
        if(args.length >= 1){
            ...
            secCount = Short.parseShort(args[0]);
            minuteAdd = (secCount - 1);
            if(secCount <= 0){
                sender.sendMessage("§cDas kann jetzt aber nicht negativ oder null sein!"); return true;
            }
            CommandBReload.seconds.add((int) secCount);
        ...
        CommandBReload.seconds.addAll(Arrays.asList(new Integer[]{10, 5, 4, 3, 2, 1, 0}));
        CommandBReload.delays.addAll(Arrays.asList(new Long[]{minuteAdd + 100, 100L,
                20L, 20L, 20L, 20L, 20L}));
