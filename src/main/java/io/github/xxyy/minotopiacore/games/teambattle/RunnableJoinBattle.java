...
public class RunnableJoinBattle implements Runnable {
    ...
    public void run() {
        if (helper.plr.getLocation().distanceSquared(helper.startLoc) > 0.5) {
            helper.plr.sendMessage(TeamBattle.chatPrefix + " §oDu kannst echt nicht 2 Sekunden stillhalten! Daher kannst du das Spiel leider nicht betreten :(");
            return;
        }
        if (helper.plr.getHealth() != helper.startHealth) {
            helper.plr.sendMessage(TeamBattle.chatPrefix + " §oNa toll, jetzt hast du während der 2 Sekunden Schaden erhalten. Daher kannst du das Spiel leider nicht betreten :(");
            return;
        }
