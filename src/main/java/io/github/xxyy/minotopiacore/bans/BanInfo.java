...
public class BanInfo {
	...
	public BanInfo(){ }
	public BanInfo(int id){ this.id = id; }//error codes...use negative values
	public BanInfo(int id,String plrName,String bannedByName,long banTimestamp,long banExpiryTimestamp,byte genericReasonId, String reason){
		this.id=id; this.plrName=plrName; this.bannedByName=bannedByName; this.banTimestamp=banTimestamp; this.banExpiryTimestamp=banExpiryTimestamp;
		this.genericReasonId=genericReasonId; this.reason=reason;
	}
	/**
	 * Saves modifications to db.
	 * @return 
	 */
	...
	public boolean nullify(){
        ...
        return false;
		if(this.id < 0) return false;
		return sql.executeUpdate("DELETE FROM "+sql.dbName+".mtc_bans WHERE id="+this.id);//no need to escape beacuse of type
	}
	/**
	...
	public static BanInfo create(String plrName, String bannedByName, String reason, byte genericReasonId, long banExpiryTimestamp){
		...
		if(!suc) return new BanInfo(-3);
		ResultSet rs = sql.executeQuery("SELECT LAST_INSERT_ID()");
		if(rs == null){ System.out.println("§4[MTC] rs == null -> db down? (1)"); return new BanInfo(-3); }
		int id = -4;
		try {
			rs.next();
			id = rs.getInt(1);
...
//		APIHelper.sendUpdateToAPI(id, (byte)2, "1");
	...
	public static BanInfo getById(int id){
		...
		if(rs == null){ System.out.println("§4[MTC] rs == null -> db down? (1)"); return new BanInfo(-3); }
		try {
			if(!rs.isBeforeFirst()) return new BanInfo(-2); //->error
			rs.next();
