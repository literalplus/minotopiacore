...
public class CommandBaninfo implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if(args.length >= 1 && !args[0].equalsIgnoreCase("help")){
			...
			if(bi.id == -2){ sender.sendMessage(MTC.banChatPrefix+"§e"+args[0]+"§c ist nicht gebannt!"); return true; }
			if(bi.id == -3){ sender.sendMessage(MTC.banChatPrefix+"SQL-Fehler! <3"); return true; }
			if(bi.id == -1){ sender.sendMessage(MTC.banChatPrefix+"Allgemeiner Fehler!"); return true; }
